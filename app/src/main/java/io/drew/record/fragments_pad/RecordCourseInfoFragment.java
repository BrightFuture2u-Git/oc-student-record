...
//    protected TextView tv_des;
    ...
    public RecordCourseInfoFragment(String courseId) {
        this.courseId = courseId;
    }

    public RecordCourseInfoFragment(int type, String courseId) {
        this.type = type;
        this.courseId = courseId;
    }
    ...
    protected void initData() {
        screenHeight = AppUtil.getScreenHeight((Activity) mContext);
        statusBarHeight = AppUtil.getStatusBarHeight(mContext);
        RetrofitManager.instance().getService(BuildConfig.API_URL, AppService.class).getRecordCourseInfo(String.valueOf(courseId)).enqueue(new BaseCallback<>(recordCourseInfo -> {
            if (recordCourseInfo != null) {
                ...
                GlideUtils.loadImg(mContext, recordCourseInfo.getCoverImage(), iv_lecture);
...
//                    new RichHtmlUtil(tv_des, recordCourseInfo.getIntroduction());
    ...
    protected void initView() {
        if (type == 2) {
            ...
            if (relay_content != null) {
                FrameLayout.LayoutParams params = (FrameLayout.LayoutParams) relay_content.getLayoutParams();
                params.leftMargin = 0;
            }
        }
        ...
        scrollView.setOnScrollChangeListener((NestedScrollView.OnScrollChangeListener) (v, scrollX, scrollY, oldScrollX, oldScrollY) -> {
            height = relay_head.getHeight()  - relay_actionbar.getHeight() - line_tab.getHeight();
            tagFlag = true;
...
//                WxShareUtil.getInstance().showShareDialog(RecordCourseInfoActivity.this, shareBitmap);
            ...
            case R.id.btn_order:
                if (EduApplication.instance.authInfo == null) {
                    ...
                    new SureOrderFragment(recordCourseInfo).show(getParentFragmentManager(), "sureOrder");
                    TagHelper.getInstance().submitEvent("btn_buy", "course_page");
                }
                break;
            case R.id.line_watch:
                new RecordCourseLecturesFragment(type,recordCourseInfo).show(getParentFragmentManager(), "recordCourseLectures");
                break;
        }
    }
}

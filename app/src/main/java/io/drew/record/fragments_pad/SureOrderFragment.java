...
//                    startActivity(intent);
                    new PayResultFragment(TextUtils.equals(resultStatus, "9000"),
                            payType,
                            aliPayOrder.getOrderId(),
                            aliPayOrder.getToken()
                    ).myShow(getParentFragmentManager(), "payResult");
                    break;
                }
                default:
                    break;
...
//                startActivity(AddAddressActivity.class);
            ...
            case R.id.tv_edit:
                if (addressList != null && addressList.getList().size() > 0) {
                    new AddressListFragment(addressList.getList()).show(getParentFragmentManager(), "addressList");
                }
                break;
    ...
    private void createWeChatOrder() {
        if (!WxShareUtil.getInstance().isWxAppInstalled(mContext)) {
